namespace BRGContainer.Runtime
{
	...
	{
		// active: skip culling, not visible. available: wait to be recycled
		...
		private static readonly int s_AvailableOffset = 4; // 1 << 4 = 16
		private static readonly uint s_bAvailableMask = 16;//0x10; // 1 << 4
		...
		[NativeDisableUnsafePtrRestriction] internal unsafe int* m_AvailableCount;

		public readonly unsafe int AvailableCount
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => *m_AvailableCount;
		}
			...
			in BatchWorldObjectData worldObjectData, Allocator allocator, bool isEmptyLODData)
		{
			...
			m_AvailableCount = (int*)UnsafeUtility.Malloc(BRGConstants.SizeOfInt, BRGConstants.AlignOfInt, m_Allocator);
			UnsafeUtility.MemClear(m_AvailableCount, BRGConstants.SizeOfInt);
			...
			for (uint lodIndex = 0u; lodIndex < lodCount; lodIndex++)
			{
				...
				else
				{
					...
					m_BatchLODs[lodIndex] = new BatchLOD(in batchDescription, in rendererDescription, in batchWorldObjectLODData, lodIndex, startOffset, m_InstanceCount,
						m_Allocator);
				}
			}
		}
		...
		public unsafe BatchLODGroup(BRGContainer container, ref BatchLODGroup oldBatchLODGroup, in BatchDescription newBatchDescription, in BatchLODGroupID batchLODGroupID)
		{
			...
			m_AvailableCount = (int*)UnsafeUtility.Malloc(BRGConstants.SizeOfInt, BRGConstants.AlignOfInt, m_Allocator);
			UnsafeUtility.MemCpy(m_AvailableCount, oldBatchLODGroup.m_AvailableCount, UnsafeUtility.SizeOf<int>());
...
// #endif
//
// 			Interlocked.Exchange(ref *m_AvailableCount, activeCount);
// 		}
		...
		public unsafe void Dispose()
		{
			...
			if ((IntPtr)m_BatchLODs == IntPtr.Zero)
				throw new InvalidOperationException($"The {nameof(BatchLODGroup)} is already disposed");
			if ((IntPtr)m_AvailableCount == IntPtr.Zero)
				throw new InvalidOperationException($"The {nameof(BatchLODGroup)} is already disposed");
#endif
			...
			if (m_Allocator > Allocator.None)
			{
				...
				UnsafeUtility.Free(m_AvailableCount, m_Allocator);
...
//             return inputDeps;
		...
		internal unsafe int SetLODRenderDataAndRegister(BatchRendererGroup batchRendererGroup, int lodIndex, GraphicsBufferHandle bufferHandle,
			in RendererDescription rendererDescription, Mesh mesh, Material[] materials, int subMeshCount)
		{
		...
		{
			uint savedState = m_State[index];
			uint savedAvailable = savedState & s_bAvailableMask;
			bool bSavedAvailable = (savedAvailable == (1u << s_AvailableOffset));
			return bSavedAvailable;
		}

		public unsafe void SetAvailable(int index, bool available)
		{
			bool savedAvailable = IsAvailable(index);
			if (!savedAvailable && available)
				(*m_AvailableCount) += 1;
			else if (savedAvailable && !available)
				(*m_AvailableCount) -= 1;
			...
			uint savedState = m_State[index];
			if (available)
				savedState |= (uint)(1u << s_AvailableOffset);
			else
				savedState &= ~(1u << s_AvailableOffset);
			m_State[index] = savedState;
		}

		public unsafe void SetAvailable(int index, uint lod, bool available)
		{
			uint savedState = m_State[index];
			bool savedAvailable = IsAvailable(index);
			if (!savedAvailable && !available)
			{
				// not a available node, do nothing
			}
			else if (savedAvailable && available)
			{
				// current is a available node, no need to update lod, do nothing
			}
			else if (savedAvailable && !available)
			{
				// if is the same lod, set unavailable, otherwise, do noting
				uint currentLOD = GetCurrentLOD(index);
				if (currentLOD == lod)
				{
			...
			{
				// a unavailable node, update lod and available state
		...
		//todo
		public unsafe (int, EGetNextActiveIndexInfo) GetNextAvailableIndex()
		{
			int index = -1;
			if (AvailableCount >= m_BatchDescription.MaxInstanceCount)
				return (index, EGetNextActiveIndexInfo.NeedResize); // need resize
			else if (AvailableCount == InstanceCount)
				index = AvailableCount; // because index starts at 0 => AvailableCount = index + 1
			else // ActiveCount < InstanceCount
			{
				for (int i = 0; i < InstanceCount; i++)
				{
					if (!IsAvailable(i)) return (i, EGetNextActiveIndexInfo.None);
				}
			}
